...
var commentsParser = function () {
  // Store reference
  var _commentsParser = function _commentsParser() {}; // Store parsers


  _commentsParser.parsers = {}; // Default detect function
  ...
  _commentsParser.normalize = function (text_block) {
    ...
    text_block = text_block.replace(/^(\s*\*+)/, ''); // Strip consistent indenting by measuring first line's whitespace
  ...
  _commentsParser.parse = function (lines, options, callback) {
    ...
    options.preserve_whitespace = !!options.preserve_whitespace; // Setup

    var current_block = '';
    var inside_single_line_block = false;
    var inside_prop_block = false;
    var inside_multi_option_prop_block = false;
    ...
    lines.split(/\n/).forEach(function (line) {
      ...
      lineNum = lineNum + 1;
      line = line + ''; // Store starting line number
      ...
      var is_multi_line_start = start_multi_line_comment(line);
      var is_multi_line_end = end_multi_line_comment(line); // Parse multi-line comments
      ...
      if (!is_multi_line_start && !inside_multi_line_block && inside_prop_block) {
      ...
      if (inside_multi_option_prop_block && end_multi_option_prop_comment(line)) {
        inside_prop_block = false;
        inside_multi_option_prop_block = false;
      } // End a one-line prop block


      if (inside_prop_block && end_prop_comment(line) && !inside_multi_option_prop_block) {
        inside_prop_block = false;
      ...
      if (!single_line_comment(line) && !inside_multi_line_block && !inside_prop_block) {
        if (current_block) {
          _blocks.push({
    ...
    _parsed = true; // Create new blocks with custom parsing

    _blocks.forEach(function (block) {
      // Store line numbers
      var from = block.from;
      var to = block.to; // Remove extra whitespace
      ...
      block.split('\n').forEach(function (line) {
        if (_commentsParser.detect(line)) {
          temp = parser(temp, _commentsParser.normalize(line), block, lines, from, to, options);
        }
  ...
  function parser(temp, line, block, file, from, to, options) {
    ...
    var name = _commentsParser.trim(parts.substr(0, index));
    ...
    contents = contents.replace(parserMarker, ''); // Redefine output contents to support multiline contents
...
commentsParser.parser('prop', function () {
  var lineOne = this.line.contents.split('split-here').slice(0, 1).join('').split(' | ');
  var lineTwo = this.line.contents.split('split-here').slice(1).map(function (str) {
    return str.trim();
  }).join('');
  var propType = lineTwo && lineTwo.match(/(PropTypes\.)([^.].*[^,])/)[2];
  return {
    library: 'react',
